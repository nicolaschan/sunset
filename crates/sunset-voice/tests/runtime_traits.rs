...
struct RecordingFrameSink {
    delivered: RefCell<Vec<(PeerId, u32, Vec<f32>)>>,
...
struct RecordingPeerStateSink {
    events: RefCell<Vec<VoicePeerState>>,
}
...
async fn traits_are_object_safe_and_implementable() {
    let d: Rc<dyn Dialer> = Rc::new(RecordingDialer {
    ...
    let p: Rc<dyn PeerStateSink> = Rc::new(RecordingPeerStateSink {
        events: RefCell::new(vec![]),
    });

    let dummy = PeerId(sunset_store::VerifyingKey::new(bytes::Bytes::from_static(
        &[0u8; 32],
    )));
