...
- Tracker task: on each presence Inserted/Replaced event, attempt to fetch the entry's block via `store.get_content(&value_hash)` and decode `PresenceBody` into an `Rc<RefCell<HashMap<PeerId, Option<String>>>>` names map alongside `presence_map`.
  ...
  - Tracker also subscribes to `Filter::All` for `Event::BlobAdded { hash }` (BlobAdded is broadcast to every subscriber regardless of key filter — see `crates/sunset-store-memory/src/subscription.rs`). On `BlobAdded`, the tracker checks whether `hash` matches any `pending_block_fetches` value; if so, retry the fetch + decode and update the names map. A separate dedicated subscription (instead of overloading the existing presence subscription) keeps the filter shape clean — `Filter::NamePrefix` doesn't match BlobAdded events anyway.
