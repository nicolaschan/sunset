...
- **Performance benchmarks.** Backfill cost on registry update equals one bloom build plus one network message — same as anti-entropy, which is already on the hot path. Worst case (forwarder with broad subscription, peer with narrow filter) is bounded by the existing per-filter digest cost.
