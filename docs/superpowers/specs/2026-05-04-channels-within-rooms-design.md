...
- **Adding a sibling field to `SignedMessage` invalidates the existing `EncryptedMessage` frozen vector** because the AEAD plaintext shape changed. We update the pinned hex/digest with a comment noting *why* we bumped (channels-within-rooms feature, 2026-05-04). The `MessageBody`-level pins (`message_body_text_postcard_hex_pin`, etc.) are unaffected — `MessageBody` itself didn't move.
